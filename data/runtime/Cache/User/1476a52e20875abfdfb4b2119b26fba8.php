			...
			<table class="table table-hover table-bordered">
				...
				<tbody>
					...
					<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
						...
						<td><?php echo ($vo['member_name']?$vo['member_name']:($vo['mobile']?$vo['mobile']:L('THIRD_PARTY_USER'))); ?></td>
						<td><?php echo ($vo['user_nicename']?$vo['user_nicename']:L('NOT_FILLED')); ?></td>
						<td><img width="25" height="25" src="<?php echo U('user/public/avatar',array('id'=>$vo['member_id']));?>" /></td>
						<td><?php echo ($vo["member_email"]); ?></td>
						<td><?php echo ($vo["member_time"]); ?></td>
						<td><?php echo ($vo["member_login_time"]); ?></td>
						<td><?php echo ($vo["member_login_ip"]); ?></td>
						<td><?php echo ($user_statuses[$vo['member_state']]); ?></td>
						<td align="center">
							<?php if(($vo["id"]) != "1"): ?><a href="<?php echo U('indexadmin/ban',array('id'=>$vo['member_id']));?>" class="js-ajax-dialog-btn" data-msg="<?php echo L('BLOCK_USER_CONFIRM_MESSAGE');?>"><?php echo L('BLOCK_USER');?></a>|
								<a href="<?php echo U('indexadmin/cancelban',array('id'=>$vo['member_id']));?>" class="js-ajax-dialog-btn" data-msg="<?php echo L('ACTIVATE_USER_CONFIRM_MESSAGE');?>"><?php echo L('ACTIVATE_USER');?></a>
							<?php else: ?>
