...
// 	    }
	    ...
	    if($result || $uc_checkemail<0 || $uc_checkusername<0){
	        ...
	        if($uc_register){
	            ...
	            $data=array(
	                ...
	                'member_time' => date("Y-m-d H:i:s"),
	                'member_login_time' => date("Y-m-d H:i:s"),
	                'member_state' => $need_email_active?2:1,
	                "user_type"=>2,//会员
	            );
	...
	public function active(){
		...
		if($find_user){
			$result=$users_model->where(array("user_activation_key"=>$hash))->save(array("user_activation_key"=>"","member_state"=>1));
			
			if($result){
				$find_user['member_state']=1;
